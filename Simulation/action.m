...
for i=1:length(person.x)
    ...
    if map(person.level(i)).action(y,x) == 1   %remove person from map
        new_person.x(i) = 0;
        new_person.y(i) = 0;
        new_person.level(i) = 0;
    end
end
new_person.x = new_person.x(find(new_person.x));   %take all the persons that have their values
new_person.y = new_person.y(find(new_person.y));   %not defined as zero
new_person.level = new_person.level(find(new_person.level));
new_person.force_x = zeros(1,length(new_person.x));
new_person.force_y = zeros(1,length(new_person.x));
